...
app.post("/paymentWithStripe", formdata, async (req, res) => {
  ...
  const myCartItems = await cartController.viewSinglePlumberCartItems(
    plumberId
  );
  ...
  const session = await references.stripe.checkout.sessions.create({
    ...
    mode: "payment",
    success_url: `http://192.168.189.75:3000/success?sc_checkout=success&sc_sid={CHECKOUT_SESSION_ID}`,
    cancel_url: `http://192.168.189.75:3000/error?sc_checkout=cancel`,
  });
